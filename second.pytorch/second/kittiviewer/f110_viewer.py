...
class KittiViewer(QMainWindow):
    ...
    def draw_detection(self, detection_anno, label_color=GLColor.Blue):
        ...
        P2 = self.kitti_info['calib/P2']
        Trv2c = self.kitti_info['calib/Tr_velo_to_cam']
        detection_anno = kitti.remove_low_height(detection_anno, 1)
        detection_anno = kitti.remove_low_score(detection_anno, 0.3)

        print('DEBUG detection_anno: ')
        for key, value in detection_anno.items() :
            print(key, value)
        print('')
        # img_filtered_annotations = {}
        # for key in detection_anno.keys():
        #     img_filtered_annotations[key] = (detection_anno[key][[0]])
        # detection_anno = img_filtered_annotations
        ...
        # print('')
        # print(dt_boxes_corners.shape)
        # print(dt_boxes_corners)
        # print('')
        # print(dt_box_color.shape)
        # print(dt_box_color)
    ...
    def load_info(self, image_idx):
        ...
        if data_input == 0:
            ...
            self.json_setting.set("latest_det_path", det_path)
            points_raw_load = np.load(det_path)
            # scale_up = 10
            points_raw_load = points_raw_load * scale_up
            points_raw = np.zeros((points_raw_load.shape[0], 4))
            points_raw[:,0:3] = points_raw_load
            points = []
            for ind in range(points_raw.shape[0]):
                if points_raw[ind, 1] < 1.2*scale_up and points_raw[ind, 1] > -1.2*scale_up and\
                    points_raw[ind, 2] < 0*scale_up and points_raw[ind, 2] > -0.6*scale_up:
                    points.append(points_raw[ind])
            points = np.array(points)
            points[:, 2] += 0.3*scale_up
            points[:, 0] += 2*scale_up
        # print('')
        # print('DEBUG: ', points.shape)
        # print('')
        print([np.min(points[:, 0]), np.min(points[:, 1]), np.min(points[:, 2]), np.max(points[:, 0]), np.max(points[:, 1]), np.max(points[:, 2])])
